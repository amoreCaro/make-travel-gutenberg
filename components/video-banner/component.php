...
Block::make(__('Video Banner', THEME))
    ...
    ->add_tab(__('Content', THEME), [
        ...
        Field::make('select', 'button_link_to', __('Button links to'))
            ->set_options([
                'blog'     => __('Blog', THEME),
                'category' => __('Category', THEME),
                'custom'   => __('Custom URL', THEME),
            ])
            ->set_default_value('blog'),

        Field::make('select', 'category', __('Category'))
            ->set_help_text(__('Used when the button links to a category.', THEME))
            ->set_options($category_options)
            ->set_conditional_logic([
                [
                    'field' => 'button_link_to',
                    'value' => 'category',
                ],
            ]),
    ...
    ->set_render_callback(function ($fields) {
        ...
        $btn_link_to = $fields['button_link_to'] ?? 'blog';
        $btn_url     = trim((string) ($fields['button_url'] ?? ''));
        $category_id = absint($fields['category'] ?? 0);
        ...
        if (!in_array($btn_link_to, ['blog', 'category', 'custom'], true)) {
            $btn_link_to = 'blog';
        }

        $posts_page_id = (int) get_option('page_for_posts');
        $blog_url = $posts_page_id
            ? (string) get_permalink($posts_page_id)
            : ((string) get_post_type_archive_link('post') ?: home_url('/blog/'));

        if ($btn_link_to === 'category' && $category_id) {
            $term_link = get_category_link($category_id);
            $btn_url = !is_wp_error($term_link) ? (string) $term_link : $blog_url;
        } elseif ($btn_link_to !== 'custom') {
            $btn_url = $blog_url;
        }
