...
Block::make('Hero Grid')
    ...
    ->add_tab('Content', [
        ...
        Field::make('textarea', 'title', 'Section Title')
            ...
            ->set_help_text('Enter the title on 2 lines. The second line will be green.'),
    ...
    ->set_render_callback(function ($fields) {
        ...
        $badge_svg_url = $badge_svg_id ? wp_get_attachment_url($badge_svg_id) : '';
        $badge_svg = $badge_svg_url ? cf_get_inline_svg($badge_svg_url) : '';
...
<section class="hero relative min-h-screen flex items-center justify-center overflow-hidden bg-[#0F0F11]">

    <!-- Background Grid -->
    <div class="absolute inset-0 grid grid-cols-3 auto-rows-fr md:grid-cols-6 md:grid-rows-3 gap-1 md:gap-2">

        <?php foreach ($bg_items as $index => $item) :

            $m_span = $mobile_patterns[$index % count($mobile_patterns)];
            $d_span = $desktop_patterns[$index % count($desktop_patterns)];
    ...
    <!-- Center Content -->
    <div class="relative z-10 max-w-4xl w-full mx-auto text-center px-4 py-12">

        <!-- Badge -->
        <?php if (!empty($badge_text) || !empty($badge_svg)) : ?>

            <span class="hero-grid__badge inline-flex items-center gap-2 px-4 py-1.5 rounded-full
                bg-white/10 backdrop-blur-md
                border border-white/20
                shadow-[0_4px_20px_rgba(0,0,0,0.15)]
                text-white text-[14px] mb-6">
                <?php if (!empty($badge_svg)) : ?>
                    <span class="inline-flex items-center justify-center w-4 h-4 shrink-0 [&>svg]:!w-4 [&>svg]:!h-4">
                        <?php echo $badge_svg; // phpcs:ignore -- trusted admin SVG markup ?>
                    </span>
                <?php endif; ?>
        ...
        <?php if (!empty($title_main) || !empty($title_accent)) : ?>
            ...
            <h1 class="text-5xl md:text-8xl font-serif font-semibold tracking-normal text-white leading-tight">
        ...
        <?php if (!empty($text)) : ?>
            ...
            <div class="mt-6 max-w-2xl mx-auto text-white/85 text-base md:text-lg  whitespace-pre-line">
    ...
    <!-- Scroll Indicator -->
    <div class="absolute bottom-8 left-1/2 -translate-x-1/2 z-10 text-white/80 text-center hidden sm:block">

        <p class="text-xs tracking-widest uppercase mb-2">
            Scroll to Explore
        </p>

        <div class="w-6 h-10 border border-white/40 rounded-full mx-auto flex justify-center pt-2">

            <span class="w-1 h-2 bg-white/80 rounded-full animate-bounce"></span>
